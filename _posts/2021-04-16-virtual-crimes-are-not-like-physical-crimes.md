...
The worst?  Possibly having attackers covertly accessing systems for months or years, while copying sensitive data from the system, utilizing the foothold gained to get access to "deeper" systems, etc.  Or even worse, subtibly changing data, payment records, etc.

Don't get me wrong - I never used the word "guilt".  Clearly, from a juridical point of view, the attacker is doing a crime, and the owner of the attacked systems is innocent.  While it may be possible to use ordinary police methods to catch the criminal in the case of a physical attack, it's impossible for a cyber attack.  It's impossible to elimiate all bad actors on the Internet, but it is possible to work on improved computer security.
