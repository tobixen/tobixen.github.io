...
The worst?  Possibly having attackers covertly accessing systems for months or years, while copying sensitive data from the system, utilizing the foothold gained to get access to "deeper" systems, etc.  Your milage may vary, if the attacker is subtibly changing data, payment records, etc without anyone noticing it may be even worse.  Breaking into a bank and stealing money from some few customer accounts, perhaps the bank wouldn't see the problem even if said customers would be complaining.

Be aware that I never used thew ord "guilt".  Clearly, from a juridical point of view, the attacker is doing a crime, and the owner of the attacked systems is innocent.  While it may be possible to use ordinary police methods to catch the criminal in the case of a physical attack, it's (almost always) impossible for a "clean" cyber attack.  It's impossible to elimiate bad actors on the Internet, but it is possible to work on improved computer security.

The real victim may not be the owner of the system, it may be the end user who trusted said system with private data.  I think it's quite *dangerous* if the owner of the system can just shrug of an attack with "it wasn't our fault, it was the fault of the attacker".
